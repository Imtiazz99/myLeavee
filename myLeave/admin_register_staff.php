<?php
	...
	{	
		...
		if(isset($_POST['submit']))
		{
			...
			if($user_type == "Admin")
			{
				$sql1 = "SELECT ADMIN_ID FROM admin WHERE ADMIN_ID = '$user_id' ";
				$result1 = mysqli_query($conn, $sql1) or die (mysqli_error());
				$row = 	mysqli_fetch_array($result1);

				if($row == 0)
				{
					$sql2 = "INSERT INTO admin (ADMIN_ID, FIRST_NAME, LAST_NAME, ADMIN_TYPE, ADMIN_DEPT, ADMIN_PASS) VALUES ('$user_id', '$user_firstName', '$user_lastName', '$user_type', '$user_dept', '$user_pass')";
				...
				else
				{
					echo "<script>
							...
							window.location=\"admin_register_staff.php\";
						  </script>";	
				}	
			}
			else if($user_type == "Staff")
			{
				$sql1 = "SELECT STAFF_ID FROM staff WHERE STAFF_ID = '$user_id' ";
				$result1 = mysqli_query($conn, $sql1) or die (mysqli_error());
				$row = mysqli_fetch_array($result1);
								
				if($row == 0)
				{
					$sql2 = "INSERT INTO staff (STAFF_ID, FIRST_NAME, LAST_NAME, STAFF_TYPE, STAFF_DEPT, STAFF_PASS) VALUES ('$user_id', '$user_firstName', '$user_lastName', '$user_type', '$user_dept', '$user_pass')";
					$result2 = mysqli_query($conn, $sql2) or die (mysqli_error());
					...
					foreach($result4 as $row4)
					{
						...
						$sql4 = "INSERT INTO leave_statistics (STAFF_ID, LEAVE_TYPE, MAX_LEAVE) VALUES ('$user_id', '$leave_type', '$no_of_days')";
						...
						<thead>
							...
							<tr>
								...
								<th>
									Enter Password: 
									<br>
            						<input type="password" name="password" id="myPassword" placeholder="Password" pattern="^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[@$!%*?&])[A-Za-z\d@$!%*?&]{8,}$" title="Minimum eight characters, at least one uppercase letter, one number and one special character" required>
								</th>
							</tr>
							<tr>
								<th>
									...
									<br>
									<input type="submit" name="submit" value="Submit" onClick="return add()">
								</th>
								<th>
...
function add() 
{
	var msg = confirm("Add This Account ?");
	if(msg == true)
		return true;
	else
		return false;
