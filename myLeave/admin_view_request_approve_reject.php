<?php
	...
	{	
		...
		$sql1 = "SELECT * FROM leave_requests WHERE STAFF_ID = '$staff_id' AND start_date = '$start_date'";
		...
		foreach($result1 as $row1)
		{
			...
			$days_requested = $row1['DAYS_REQUESTED'];
			$date_applied = $row1['DATE_APPLIED'];
			$status = $row1['LEAVE_STATUS'];
		}
		...
		{
			$staff_firstName = $row2['FIRST_NAME'];
			$staff_lastName = $row2['LAST_NAME'];
			$staff_dept = $row2['STAFF_DEPT'];
		}
	}
?>
...
<body>
		...
		<div class="dropdown">
			<button onclick="myFunction()" class="dropbtn"></button>
  			<div id="myDropdown" class="dropdown-content">
				...
				<a href="logout.php" onClick="return message()">Logout</a>
  			</div>
		</div>
    </div>
  </header>
  ...
  <main class="main">
					...
					<thead>
						<tr>
							<th>
								Staff ID:
								<br>
								<input type="text" name="id" value=" <?php echo $staff_id ?>" readonly>
							</th>
							<th>
								Staff Department: 
								<br>
								<input type="text" name="department" value="<?php echo $staff_dept ?>" readonly>
							</th>
						</tr>
						<tr>
							<th>
								Staff First Name: 
								<br>
								<input type="text" name="firstName" value="<?php echo $staff_firstName ?>" readonly>
							</th>
							<th>
								Staff Last Name: 
								<br>
								<input type="text" name="lastName" value="<?php echo $staff_lastName ?>" readonly>
							</th>
						</tr>
					</thead>
				</table>
	...
	<div class="card">
		...
		<div class="card__main">
			<div class="card__row">
				<form action="admin_view_request_approve_reject_db.php" method="post">
				<table>
					<thead>
						<tr>
							<th>Leave Types: </th>
							<th>
								<input type="text" name="leaveType" value="<?php echo $leave_type ?>" readonly/>
							</th>
                        </tr>
						<tr>
							<th>Date Applied: </th>
							<th>
								<input type="text" name="dateApplied" value="<?php echo $date_applied ?>" readonly/>
							</th>
						</tr>
						    <th>Start Date: </th>
							<th>
								<input type="text" name="startDate" value="<?php echo $start_date ?>" readonly/
							</th>
						<tr>
							<th>End Date: </th>
							<th>
								<input type="text" name="endDate" value="<?php echo $end_date ?>" readonly/>
							</th>
						</tr>
						<tr>
							<th>Total Days: </th>
							<th>
								<input type="text" name="daysRequested" value="<?php echo $days_requested ?> Day(s)" readonly/>
							</th>
						</tr>
						<tr>
							<th>Action: </th>
							<th>
								<select name="approveReject">
						...
						<tr>
							...
							<th>
								<a href="admin_view_request.php">Cancel</a>
							</th>
						</tr>
                    </thead>
				</table>
