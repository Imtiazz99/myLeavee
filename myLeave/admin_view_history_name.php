<?php
	...
	{	
		...
		$sql1 = "SELECT * FROM staff WHERE FIRST_NAME LIKE '%$staff_name%' OR LAST_NAME LIKE '%$staff_name%'" ;
		$result1 = mysqli_query($conn, $sql1) or die (mysqli_error());
		
		foreach($result1 as $row1)
		{
			$staff_id = $row1['STAFF_ID'];
			$staff_firstName = $row1['FIRST_NAME'];
			$staff_lastName = $row1['LAST_NAME'];
			$staff_dept = $row1['STAFF_DEPT'];
		}	
		
		$sql2 = "SELECT * FROM leave_requests WHERE STAFF_ID = '$staff_id'";
		$result2 = mysqli_query($conn, $sql2) or die (mysqli_error());
		
		$sql3 = "SELECT * FROM leave_statistics WHERE STAFF_ID = '$staff_id'";
		$result3 = mysqli_query($conn, $sql3) or die (mysqli_error());
		
		if(mysqli_num_rows($result1) == 0)
		{
			echo "<script>alert(\"Staff Does Not Exist !\");
					window.location=\"admin_view_history.php\";</script>";
		}
	}
?>
    ...
    <div class="main-header">
	 ...
	 <div class="card">
		...
		<div class="card__main">
			<div class="card__row">
				<form action="admin_update_staff_details_db.php" method="post">
					<table>
						<thead>
							<tr>
								<th>
									Staff ID:
									<br>
									<input type="text" value=" <?php echo $staff_id ?>" readonly>
								</th>
								<th>
									Staff Department: 
									<br>
			...
			<div class="card__row">
				<table>
					...
					<?php
						...
						{
							$leave_type = $row3['LEAVE_TYPE'];
							$maximum_leaves = $row3['MAX_LEAVE'];
							$leaves_taken = $row3['LEAVE_TAKEN'];
					...
					?>
						<r>
							<td>
								<a href="admin_view_history.php"><button class="backBtn" style="margin-bottom: 5px;">Back</button></a>
							</td>
							<td></td>
							<td></td>
							<td></td>
